...
auto CheckJoinPredicate(const AbstractExpression *predicate,
                        std::vector<std::shared_ptr<AbstractExpression>> &left_key_expressions,
                        std::vector<std::shared_ptr<AbstractExpression>> &right_key_expressions) -> bool {
  if (const auto *expr = dynamic_cast<const ComparisonExpression *>(predicate); expr != nullptr) {
...
auto Optimizer::OptimizeNLJAsHashJoin(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  if (optimized_plan->GetType() == PlanType::NestedLoopJoin) {
    ...
    std::vector<std::shared_ptr<AbstractExpression>> left_key_expressions;
    std::vector<std::shared_ptr<AbstractExpression>> right_key_expressions;

    if (CheckJoinPredicate(nlj_plan.Predicate().get(), left_key_expressions, right_key_expressions)) {
    ...
    if (const auto *expr = dynamic_cast<const LogicExpression *>(nlj_plan.Predicate().get()); expr != nullptr) {
      if (expr->logic_type_ == LogicType::And) {
        if (CheckJoinPredicate(expr->children_[0].get(), left_key_expressions, right_key_expressions) &&
            CheckJoinPredicate(expr->children_[1].get(), left_key_expressions, right_key_expressions)) {
          return std::make_shared<HashJoinPlanNode>(nlj_plan.output_schema_, nlj_plan.GetLeftPlan(),
